namespace LCPD_First_Response.LCPDFR.Callouts
{
    ...
    /// </summary>
    [CalloutInfo("Robbery", ECalloutProbability.Always)]
    internal class Robbery : Callout
    {
        ...
        /// <summary>
        /// Vehicle models that can be used for the robbery call.
        /// </summary>
        private string[] robberyModels = new string[] { "SPEEDO", "CAVALCADE", "REBLA", "STOCKADE", "PONY" };
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="Robbery"/> class.
        /// </summary>
        public Robbery()
        {
            // Determine type
            ...
            if (this.calloutType == ECalloutType.GrandTheftAuto)
            {
                ...
                {
                    case 0:
                        Functions.PlaySoundUsingPosition("UNITS_PLEASE_BE_ADVISED INS_WE_HAVE_A_REPORT_OF_ERRR CRIM_A_GRAND_THEFT_AUTO IN_OR_ON_POSITION", this.spawnPosition);
                        break;
                    case 1:
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        Functions.PlaySoundUsingPosition("INS_UNITS_REPORT CRIM_A_SUSPECT_RESISTING_ARREST IN_OR_ON_POSITION", this.spawnPosition);
                        break;
                    case 4:
            ...
            else if (this.calloutType == ECalloutType.Robbery)
            {
                ...
                {
                    case 0:
                        Functions.PlaySoundUsingPosition("ALL_UNITS_ALL_UNITS INS_WE_HAVE CRIM_A_CRIMINAL_PERFORMING_DRIVEBY_ACTIVITY IN_OR_ON_POSITION", this.spawnPosition);
                        break;
                    case 1:
                        Functions.PlaySoundUsingPosition("ALL_UNITS INS_WEVE_GOT CRIM_CRIMINALS_PERFORMING_DRIVEBY_ACTIVITY IN_OR_ON_POSITION", this.spawnPosition);
                        break;
                    case 2:
                        Functions.PlaySoundUsingPosition("UNITS_REPORT CRIM_CRIMINALS_PERFORMING_DRIVEBY_ACTIVITY IN_OR_ON_POSITION", this.spawnPosition);
                        break;
                }
            }
        }
        ...
        public override bool OnCalloutAccepted()
        {
            ...
            {
                this.vehicle = new LVehicle(World.GetNextPositionOnStreet(this.spawnPosition), Common.GetRandomCollectionValue<string>(this.robberyModels));
            }
            else
            {
            ...
            if (this.vehicle.Exists())
            {
                ...
                int peds = Common.GetRandomValue(1, 3); // 1 or 2
                if (this.calloutType == ECalloutType.Robbery)
                {
                    // 2, 3 or 4.
                    peds = Common.GetRandomValue(2, 5);
                }
                ...
                for (int i = 0; i < this.robbers.Length; i++)
                {
                    ...
                    if (this.robbers[i].Exists())
                    {
                        ...
                        if (!this.vehicle.HasDriver)
                        {
                            ...
                            if (this.calloutType == ECalloutType.Robbery)
                            {
                                // If a robbery, make all passengers able to perform drivebys
                                this.robbers[i].Ped.WillLeaveCarInCombat = false;
                                this.robbers[i].WillUseCarsInCombat = true;
                            }
                        }
                        else
                        {
                            ...
                            if (this.calloutType == ECalloutType.Robbery)
                            {
                                ...
                                this.robbers[i].Ped.WillLeaveCarInCombat = false;
                                this.robbers[i].WillUseCarsInCombat = true;
                                this.robbers[i].WillDoDrivebys = true;
                            }
                        }
                ...
                // Dispatch another unit to help
                Functions.RequestPoliceBackupAtPosition(this.vehicle.Position);
                ...
                //    dynamicData.Write(Functions.GetStringFromLanguageFile("CALLOUT_ROBBERY_CATCH_UP"));
                //    dynamicData.Write(25000);
                //    Engine.Main.NetworkManager.ActivePeer.Send("Callouts", ECalloutNetworkMessages.CalloutInstruction, dynamicData);
                //}
            }
            else
            {
        ...
        public override void Process()
        {
            ...
            int arrestCount = this.robbers.Count(robber => robber != null && robber.Exists() && robber.HasBeenArrested);
